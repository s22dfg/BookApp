...
//  BookApp
//
//  Created by D7703_22 on 2018. 11. 12..
...
class MainTableViewController: UITableViewController {
    ...
    func search(query:String, page:Int){
        ...
        if let strURL = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed){
            if let url = URL(string: strURL){
                ...
                request.addValue("KakaoAK b5e32ec179fdd2a2d0d0d28b647ba086", forHTTPHeaderField: "Authorization")
    ...
    func handler(data:Data?, response:URLResponse?, error:Error?) -> Void {
        ...
        if let dot = data {
            do{
                if let dic = try JSONSerialization.jsonObject(with: dot, options: [])as? [String:Any]{
